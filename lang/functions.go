...
func (ctx *Context) Functions() map[string]function.Function {
	ctx.funcsLock.Lock()
	if ctx.funcs == nil {
		ctx.funcs = map[string]function.Function{
		...
		if ctx.PureOnly {
			...
			for _, name := range impureFunctions {
				ctx.funcs[name] = function.Unpredictable(ctx.funcs[name])
			}
		}
	}
	ctx.funcsLock.Unlock()
